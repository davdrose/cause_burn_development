...
library("marginaleffects") # for marginaleffects
library("emmeans")         # for marginaleffects
...
- difference on the response scale (which ranges from 0 to 1)

```{r}
fit.brm.chain.cause.adults %>% 
  emmeans(consec ~ question, 
          type = "response")
```
...
df.model = df.made_children_different %>%
  mutate(difference = distal - proximal,
        difference = recode(difference, `-1` = 0))
...
df.model = df.cause_children_different %>%
