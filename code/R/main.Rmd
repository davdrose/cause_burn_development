...
df.plot = df.exp2_cause_adults %>% 
  ...
  mutate(response_pattern = case_when(
    ...
    direct == 0 & absence == 1  ~ "absence cause only",
  ),
  question = recode(question, "cause" = "caused"),
  age_whole = as.factor(age_whole)) %>% 
  group_by(age_whole, question, response_pattern) %>% 
  ...
  ungroup() %>% 
  mutate(question = factor(question, levels = c("lexical", "caused")),
         response_pattern = factor(response_pattern, levels = c("absence cause only", "direct cause only", "both causes", "neither cause")))
...
p7 = ggplot(data = df.plot,
  ...
  facet_wrap(~question, 
             scales = "free_x") + 
  geom_col(color = "black") +
 scale_fill_manual(values = c("absence cause only" = "#E41A1C",
                               "direct cause only" = "#377EB8",
                               "both causes" = "#984EA3",
                               "neither cause" = "#999999")) +
  scale_color_manual(values = c("absence cause only" = "#E41A1C",
                                "direct cause only" = "#377EB8",
                                "both causes" = "#984EA3",
                                "neither cause" = "#999999")) +
  scale_shape_manual(values = c(21, 23)) +
...
df.plot = df.exp3_adults %>% 
  ...
  mutate(percentage = n / sum(n) * 100) %>%
  ungroup() %>% 
  mutate(response_pattern = factor(response_pattern, levels = c("absence cause only", "direct cause only", "both causes", "neither cause"))) %>% 
...
#### Combine result and parameters plot

```{r, fig.width=16, fig.height=5}

plot = rsa.overall.plot + rsa.sub.plot + 
plot_layout(nrow = 1) &
plot_annotation(tag_levels = "A") &
theme(plot.tag = element_text(face = "bold"))

ggsave(plot, filename = "../../figures/rsa/rsa_combined.pdf", height = 5, width = 16)
